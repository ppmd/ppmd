...
if __name__ == '__main__':
    ...
    initial_charges = rng.uniform(-1.0, 1.0, (N, 1))
    ...
    fmm = coulomb.fmm.PyFMM(A.domain, free_space=False, r=max(3, math.log(N, 8)), l=12)
    ...
    for it in IntegratorRange(steps, dt, A.vel, shell_steps, delta, verbose=False):
        ...
        vv1.execute()
        lj_pairloop.execute()
        elec_energy = fmm(A.pos, A.charge, A.force)
        vv2.execute()
        
        if A.domain.comm.rank == 0 and it % 10 == 0:
            print("{: 8d} | {: 12.8e} {: 12.8e} | {: 12.8e}".format(it, lj_energy[0], ke_energy[0], lj_energy[0] + ke_energy[0] + elec_energy))
