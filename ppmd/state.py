...
class _move_controller(object):
    ...
    def move_to_neighbour(self, ids_directions_list=None, dir_send_totals=None, shifts=None):
        ...
        self._move_unpacking_lib.execute(
            static_args={'_recv_count': ctypes.c_int(_recv_total),
                        ...
                        '_prev_num_particles': ctypes.c_int(self.state.npart_local)}
        )
    ...
    def _move_build_packing_lib(self):
        ...
        for idx, ix in enumerate(self.state.particle_dats):
            # existing dat in state
            self._packing_args_shift['%(NAME)s' % {'NAME':ix}] = getattr(self.state, ix)
    ...
    def _compress_particle_dats(self, num_slots_to_fill):
        ...
        if self.compressed is True:
            ...
            self._compressing_lib.execute(
                static_args={
                    'slots_to_fill_in': ctypes.c_int(num_slots_to_fill),
                    'n_new_in': ctypes.c_int(self.state.npart_local)
                },
