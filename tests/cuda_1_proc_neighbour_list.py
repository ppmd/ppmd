...
sim1.state.h_forces = data.ParticleDat(npart=sim1.state.d_positions.npart, ncomp=3, dtype=ctypes.c_double)
...
for ix in range(N):
    err = np.absolute(sim1.state.h_forces.dat[ix,::] - sim1.state.forces.dat[ix,::])
    print err, sim1.state.h_forces.dat[ix,::], sim1.state.forces.dat[ix,::]
