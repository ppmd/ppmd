...
for ix in range(N):
    err = np.linalg.norm(sim1.state.h_positions.data[ix,::] - sim1.state.positions.data[ix,::])

    if err > 10 ** (-4):
        passed = False
        print ix, err, sim1.state.h_positions.data[ix,::], sim1.state.positions.data[ix,::]
