...
import ppmd.sycl.sycl_particle_loop
SYCLLOOP = ppmd.sycl.sycl_particle_loop.SYCLParticleLoopBasic
...
def test_sycl_red_1():
    ...
    s_nd = State()
    s_nd.npart = N
    s_nd.p = PositionDat(ncomp=3)
    s_nd.v = ParticleDat(ncomp=3)
    s_nd.f = ParticleDat(ncomp=3)
    s_nd.gid = ParticleDat(ncomp=1, dtype=ctypes.c_int)
    s_nd.u = GlobalArray(size=1)


    gid_sum = GlobalArray(size=1, dtype=ctypes.c_int)
    ...
    kernel = md.kernel.Kernel('test_sycl_red_1',code=kernel_code)
    kernel_map = {'P': s_nd.p(md.access.READ),
                  'gid_sum': gid_sum(INC_ZERO)}

    loop = SYCLLOOP(kernel=kernel, dat_dict=kernel_map)
    loop.execute(n=N)


    assert gid_sum[0] == N, "critical GlobalArray Failure"



def test_sycl_looping_1(s_nd):
    """
    ...
    kernel = md.kernel.Kernel('test_sycl_looping_1',code=kernel_code)
...
# will not currently compile with dpc++
@pytest.mark.skipif("True")
def test_sycl_looping_2(s_nd):
    """
    looping on non spatially decomposed state
    """
